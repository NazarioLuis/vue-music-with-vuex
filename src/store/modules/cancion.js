...
const getters = {
    ...
    getFavoritosCopy:(state) => {
        return JSON.parse(JSON.stringify(state.favoritos))
    }
}
const mutations = {
    setAlbum:(state, album) => state.album = album,
    setCanciones:(state, canciones) => state.canciones = canciones,
    setFavoritos:(state, canciones) => state.favoritos = canciones,
    setCancionSeleccionada:(state, cancion) => state.cancionSeleccionada = cancion,
}
...
const actions= {
    ...
    seleccionarCancion: ({commit},cancion) => commit("setCancionSeleccionada",cancion),
